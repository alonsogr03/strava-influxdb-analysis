...
Se prosigue calculando, para cada usuario, el número de actividades, la distancia total realizada y la duración total en minutos.
...
```{python}
# Ordenamos el DataFrame para el podio visual (2º, 1º, 3º)
# Esto es crucial para que la barra más alta quede en el centro.
if len(df) == 3:
...
# Definir los colores de las medallas (Plata, Oro, Bronce)
# Se asignan al índice del df_podio reordenado
colores_medallas = ['#C0C0C0', '#FFD700', '#CD7F32'] if len(df_podio) == 3 else ['#FFD700', '#C0C0C0'] if len(df_podio) == 2 else ['#FFD700']

plt.figure(figsize=(10, 6))

# Crear las barras. Usamos el índice como etiqueta del eje X para que sean 3 barras separadas.
# Luego pondremos el nombre del usuario encima/dentro
bars = plt.bar(range(len(df_podio)), df_podio['distancia_max'], color=colores_medallas, edgecolor='black')

# Añadir etiquetas de distancia y nombre del usuario
for i, bar in enumerate(bars):
    yval = bar.get_height()
    usuario = df_podio.loc[i, 'usuario']
    distancia = df_podio.loc[i, 'distancia_max']
    
    # Etiqueta de la distancia encima de la barra
    plt.text(bar.get_x() + bar.get_width()/2, yval + 0.1, 
             f'{distancia:.2f} km', ha='center', va='bottom', fontweight='bold', fontsize=12)
    
    # Etiqueta del usuario dentro de la barra (con posición)
    puesto = ""
    if len(df_podio) == 3:
        if i == 1: puesto = "1º\n" # Oro (centro)
        elif i == 0: puesto = "2º\n" # Plata (izquierda)
...
plt.xticks([]) 
# Quitar líneas de los ejes para que parezca un podio limpio
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['left'].set_visible(False)
plt.yticks([]) # Ocultar números del eje Y
...
# 1. Reordenar el DataFrame para el efecto visual de podio (2º, 1º, 3º)
if len(df) == 3:
    df_podio = df.iloc[[1, 0, 2]].reset_index(drop=True)
else:
    df_podio = df.sort_values('eficiencia_aerobica_sesion', ascending=False)

# 2. Configuración de colores (Plata, Oro, Bronce)
colores_medallas = ['#C0C0C0', '#FFD700', '#CD7F32']

plt.figure(figsize=(10, 7))

# 3. Dibujar las barras individuales
...
# 4. Añadir la información dentro y sobre las barras
for i, bar in enumerate(bars):
    yval = bar.get_height()
    usuario = df_podio.loc[i, 'usuario']
    
    # Etiqueta del valor exacto encima de la barra
    plt.text(bar.get_x() + bar.get_width()/2, yval + (yval * 0.02), 
             f'{yval:.4f}\nm/latido', ha='center', va='bottom', 
             fontweight='bold', fontsize=11, color='#2c3e50')
    
    # Determinar el puesto según el valor original
    # (Comparamos con el DF original que está ordenado de mayor a menor)
    ...
    # Texto dentro de la barra: Puesto y Nombre del Usuario
    plt.text(bar.get_x() + bar.get_width()/2, yval / 2, 
             f"{puesto}\n\n{usuario}", 
             ha='center', va='center', color='black', 
             fontsize=14, fontweight='black')

# 5. Estética final estilo "Clean Podium"
plt.title('Podio de Eficiencia Aeróbica (Media por Sesión)', fontsize=16, pad=30, fontweight='bold')
plt.ylim(0, df['eficiencia_aerobica_sesion'].max() * 1.3) # Espacio para las etiquetas
